...
const PAGE_TITLE = 'Barcode View Report';
const MAX_FLT_DISTINCT = 500;
const MAX_FILTERS = 6;
const SKIP_FILTERS = ['color', 'jc', 'hsncode', 'status'];
...
function allColumnKeys() {
    return Array.isArray(window.__BV_COLUMNS__) ? window.__BV_COLUMNS__ : [];
}
...
function buildMeta(rows) {
    ...
    meta.all.forEach(k => {
        const t = (window.__BV_TYPES__ && window.__BV_TYPES__[k]) || detectTypeFromData(rows, k);
    ...
    meta.filters = [];
    meta.all.forEach(k => {
        if (meta.filters.length >= MAX_FILTERS) return;
        const kl = k.toLowerCase();
        if (meta.numSet[kl] || meta.dateSet[kl]) return;
        if (kl === 'id' || kl.endsWith('_id')) return;
        if (SKIP_FILTERS.indexOf(kl) >= 0) return;
        const values = rows.map(r => rowGet(r, k)).filter(v => v != null && v !== '');
        if (!values.length) return;
        const distinct = new Set(values.map(v => String(v).toLowerCase())).size;
        if (distinct > MAX_FLT_DISTINCT) return;
...
function buildAutoFilters() {
    ...
    searchWrap.innerHTML =
        '<input type="text" id="search-client" class="form-control ps-5" placeholder="Search records...">' +
    ...
    dateWrap.id = 'date-range-block';
    dateWrap.className = 'flt-date-wrap d-flex flex-wrap gap-2 align-items-end';
    dateWrap.innerHTML =
        '<div class="date-filter-col">' +
        '  <label class="form-label mb-1" for="filter-from">From Date</label>' +
        '  <input type="date" id="filter-from" class="form-control form-control-sm date-input">' +
        '</div>' +
        '<div class="date-filter-col">' +
        '  <label class="form-label mb-1" for="filter-to">To Date</label>' +
        '  <input type="date" id="filter-to" class="form-control form-control-sm date-input">' +
        '</div>';
    ...
    if (fromEl && !fromEl.dataset.bound) {
        ...
        fromEl.addEventListener('change', function () {
            if (this.value && toEl && toEl.value && this.value > toEl.value) toEl.value = '';
            applyFilters();
        });
    }
    if (toEl && !toEl.dataset.bound) {
        ...
        toEl.addEventListener('change', function () {
            if (this.value && fromEl && fromEl.value && this.value < fromEl.value) fromEl.value = '';
            applyFilters();
        });
    }

    const btns = document.createElement('div');
    btns.className = 'flt-btn-wrap d-flex flex-wrap gap-2 align-items-center ms-auto';
    btns.innerHTML =
        '<button type="button" class="btn btn-light btn-action" id="btn-clear"><i class="ri-refresh-line me-1"></i>Clear</button>' +
        '<button type="button" class="btn btn-action btn-excel" id="btn-excel"><i class="ri-file-excel-line me-1"></i>Excel</button>' +
        '<button type="button" class="btn btn-action btn-csv" id="btn-csv"><i class="ri-file-list-line me-1"></i>CSV</button>' +
        '<button type="button" class="btn btn-action btn-print" id="btn-print"><i class="ri-printer-line me-1"></i>Print</button>';
    ...
    const bExcel = $('btn-excel');
    if (bExcel && !bExcel.dataset.bound) {
        bExcel.dataset.bound = '1';
        bExcel.addEventListener('click', function () { exportExcel(getFiltered()); });
    }
    const bCsv = $('btn-csv');
    if (bCsv && !bCsv.dataset.bound) {
        bCsv.dataset.bound = '1';
        bCsv.addEventListener('click', function () { exportCsv(getFiltered(), 'BarcodeViewReport.csv'); });
    }
    const bPrint = $('btn-print');
    if (bPrint && !bPrint.dataset.bound) {
        bPrint.dataset.bound = '1';
        bPrint.addEventListener('click', printTable);
    }

    if (!meta.filters.length) {
        wrap.insertAdjacentHTML('beforeend', '<div class="text-muted fs-13 py-1">No filter options detected.</div>');
    }
}
...
function loadData() {
    ...
    if (Array.isArray(window.__BV_DATA__)) {
        applyData(window.__BV_DATA__);
        return;
    }
    if (window.__BV_ERROR__) {
        $('loading').classList.add('d-none');
        $('item-grid').classList.remove('d-none');
        toast('Data load failed: ' + window.__BV_ERROR__, true);
        return;
    }

    fetch('/BarcodeView/GetData').then(r => r.json())
        .then((res) => {
            ...
            window.__BV_COLUMNS__ = (res && Array.isArray(res.columns) && res.columns.length)
                ? res.columns : window.__BV_COLUMNS__;
            if (res && res.types) window.__BV_TYPES__ = res.types;
            applyData(rows);
        })
        .catch(err => {
            $('loading').classList.add('d-none');
...
function viewItem(key) {
    ...
    $('bv-view-body').innerHTML = html || '<div class="col-12 text-center text-muted py-3">No fields found.</div>';
...
function exportExcel(rows) {
    download('BarcodeViewReport.xls', '\uFEFF' + headerRows(), 'application/vnd.ms-excel');
}
