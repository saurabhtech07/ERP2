...
const JSBC_CONFIG = {
    title: 'Jobcard Stage Bal Cut',
    controller: 'JobcardStageBalCut',
    columns: [
        { key: 'docno', label: 'DOC NO', type: 'bold' },
    ...
    filters: [
        { key: 'docno', mount: 'fsd-docno', ph: 'Job No', accent: '', icon: 'ri-file-user-line' },
        { key: 'cat1', mount: 'fsd-cat1', ph: 'Category', accent: '', icon: 'ri-price-tag-3-fill' }
    ],
    stats: [
        { el: 'stat-total', label: 'Total Records', kind: 'count' },
        { el: 'stat-qty', label: 'Total Qty', kind: 'sum', key: 'totalqty' },
        { el: 'stat-days', label: 'Days', kind: 'distinct', key: 'date' },
        { el: 'stat-jobs', label: 'Jobs', kind: 'distinct', key: 'docno' }
    ],
    searchFields: [],
    numericKeys: ['totalqty'],
    autoFilterLimit: 30,
    skipAutoFilter: ['id', 'actions', '__rowmapindex', 'name', 'date', 'totalqty']
};
...
function allColumnKeys() {
    if (Array.isArray(window.__JSBC_COLUMNS__) && window.__JSBC_COLUMNS__.length) return window.__JSBC_COLUMNS__;
    return JSBC_CONFIG.columns.map(c => c.key);
}
...
function syncFilterOptions() {
    JSBC_CONFIG.filters.forEach(function (f) {
...
function buildAutoFilters(rows) {
    ...
    const existing = {};
    JSBC_CONFIG.filters.forEach(f => existing[String(f.key).toLowerCase()] = true);

    keys.forEach(function (key) {
        const raw = String(key);
        const k = raw.toLowerCase();
        if (existing[k]) return;
        if (JSBC_CONFIG.skipAutoFilter.indexOf(k) >= 0) return;

        const vals = unique(rows.map(r => rowGet(r, k)));
        if (vals.length === 0 || vals.length > JSBC_CONFIG.autoFilterLimit) return;
        if (vals.every(v => v !== '' && !isNaN(Number(v)))) return;
        ...
        if (!mount) {
            ...
            holder.className = 'col-6 col-lg-3 fsd-auto-col';
            holder.innerHTML = '<div id="' + mountId + '" class="fsd-mount"></div>';
            const holderRow = document.querySelector('.filter-panel .row.g-2.align-items-end');
            if (!holderRow) return;
            holderRow.appendChild(holder);
            mount = $(mountId);
        }
        if (flts[k] || autoFltDone[k]) return;
        autoFltDone[k] = true;

        const f = { key: k, mount: mountId, ph: titleCase(raw), accent: accent, icon: 'ri-filter-3-fill', auto: true };
        JSBC_CONFIG.filters.push(f);
...
function inDateRange(r) {
    const dFrom = $('from-date').value;
    const dTo = $('to-date').value;
...
function getFiltered() {
    ...
    return allRows.filter(function (r) {
        for (let i = 0; i < JSBC_CONFIG.filters.length; i++) {
            const f = JSBC_CONFIG.filters[i];
...
function computeStats(rows) {
    JSBC_CONFIG.stats.forEach(function (s) {
        const el = $(s.el);
        if (!el) return;
        let n = 0;
        if (s.kind === 'count') {
            ...
            n = new Set(rows.map(function (r) { return rowGet(r, s.key); }).filter(function (v) { return v != null && v !== ''; })).size;
        }
        animateCount(el, n);
    });
}
...
function applyData(rows) {
    ...
    try { buildAutoFilters(rows); } catch (e) { console.error('buildAutoFilters:', e); }
    try { syncFilterOptions(); } catch (e) { console.error('syncFilterOptions:', e); }
    computeStats(rows);
    refreshTable(getFiltered());
    $('loading').classList.add('d-none');
    $('jsbc-grid').classList.remove('d-none');
}

function loadData() {
    $('loading').classList.remove('d-none');
    $('jsbc-grid').classList.add('d-none');

    if (Array.isArray(window.__JSBC_DATA__)) {
        applyData(window.__JSBC_DATA__);
        return;
    }
    if (window.__JSBC_ERROR__) {
        $('loading').classList.add('d-none');
        $('jsbc-grid').classList.remove('d-none');
        toast('Data load failed: ' + window.__JSBC_ERROR__, true);
        return;
    }

    fetch('/' + JSBC_CONFIG.controller + '/Items').then(r => r.json())
        .then((res) => {
            ...
            window.__JSBC_COLUMNS__ = (res && Array.isArray(res.columns) && res.columns.length)
                ? res.columns : window.__JSBC_COLUMNS__;
            buildGrid();
            applyData(rows);
        })
        .catch(err => {
...
function buildGridColumns() {
    const keys = allColumnKeys();
    const known = {};
    JSBC_CONFIG.columns.forEach(c => known[c.key] = c);
    ...
    keys.forEach(function (key) {
        ...
        const kc = known[k];
        const label = (kc && kc.label) ? kc.label : titleCase(raw);
...
function viewItem(idx) {
    ...
    keys.forEach(function (key) {
        ...
        const kc = (JSBC_CONFIG.columns.find(c => c.key === k) || {});
        const label = (kc.label) ? kc.label : titleCase(raw);
    ...
    const title = firstVal != null ? 'Jobcard Stage Balance - ' + firstVal : 'Jobcard Stage Balance';
...
function exportExcel(rows) {
    download('JobcardStageBalCut.xls', '\uFEFF' + headerRows(), 'application/vnd.ms-excel');
}
...
function printTable() {
    ...
    w.document.write('<!DOCTYPE html><html><head><title>' + esc(JSBC_CONFIG.title) + '</title>');
    ...
    w.document.write('<h2>' + esc(JSBC_CONFIG.title) + ' Report</h2>');
...
$('btn-csv').addEventListener('click', function () { exportCsv(getFiltered(), 'JobcardStageBalCut.csv'); });
